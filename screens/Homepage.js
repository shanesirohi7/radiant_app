...
const App = ({ onLogout }) => {
    ...
    const fetchMemories = async (token) => {
        try {
            const response = await axios.get(`${API_URL}/memories`, { headers: { token } });
        ...
        <View style={styles.memoryItem}>
            <View style={styles.memoryHeader}>
                <View style={styles.memoryUser}>
                    <Image source={{ uri: item.user.profilePic }} style={styles.memoryUserAvatar} />
                    <View>
                        <Text style={styles.memoryUserName}>{item.user.name}</Text>
                        <View style={styles.memoryInfo}>
                            <Text style={styles.memoryTitle}>{item.title}</Text>
                            <Text style={styles.memoryDot}>•</Text>
                            <Text style={styles.memoryContributors}>
                                with {item.contributors.slice(0, 2).map(contributor => contributor.name).join(', ')}
                                {item.contributors.length > 2 ? ` +${item.contributors.length - 2} more` : ''}
                            </Text>
                        </View>
                    </View>
                </View>
            ...
            <View style={styles.memoryStats}>
                <Text style={styles.memoryLikes}>{item.likes} likes</Text>
                <TouchableOpacity>
                    <Text style={styles.memoryViewComments}>View all {item.comments.length} comments</Text>
                </TouchableOpacity>
                <Text style={styles.memoryTimestamp}>{item.createdAt}</Text>
            </View>
        </View>
    );
...
/>
        ...
        <View style={styles.bottomNav}>
            ...
            <TouchableOpacity style={styles.navItem} onPress={() => nav.navigate('Profile')}>
                <Image source={{ uri: user?.profilePic || '[https://via.placeholder.com/24](https://via.placeholder.com/24)' }} style={styles.navProfilePic} />
            </TouchableOpacity>
        </View>
